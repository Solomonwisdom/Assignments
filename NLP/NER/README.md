...
1. Download the sgns vectors with

```
make sgns
```

Alternatively, you can download them manually [here](https://nlp.stanford.edu/projects/sgns/) and update the `sgns_filename` entry in `config.py`. You can also choose not to load pretrained word vectors by changing the entry `use_pretrained` to `False` in `model/config.py`.
...
1. [DO NOT MISS THIS STEP] Build vocab from the data and extract trimmed sgns vectors according to the config in `model/config.py`.
